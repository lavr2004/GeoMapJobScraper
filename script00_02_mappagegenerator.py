...
def getcode_map_full2(vacancies):
    ...
    <body>
        ...
        function filterMarkers(type) {{
            markers.forEach(({{ marker, is_new }}) => {{
                if (type === 'new' && !is_new) map.removeLayer(marker.marker);
                else if (type === 'old' && is_new) map.removeLayer(marker.marker);
                else if (!map.hasLayer(marker.marker)) map.addLayer(marker.marker);
            }});
        }}
    ...
    <body>
        ...
        function addMarkers(minSalary, maxDistance, searchText = '', excludeText = '', selectedSource = 'all', dateFrom = '', dateTo = '') {{
            ...
            vacancies.forEach(vacancy => {{
                ...
                let matchesTag = textFiltersActive ? activeTags.size === 0 : true;
                if (textFiltersActive) {{
                    for (let tag of activeTags) {{
                        if (title.includes(tag.toLowerCase())) {{
                            matchesTag = true;
