...
class PracujPLParser(BaseParser):
    ...
    def parse(self, pagenumber_int=0) -> None:
        ...
        # Save JSON copy
        bin.logic.filesystem.save_jobs_copy_into_json_file_fc(self.results_filepath_str, jobs)

        # Save to database
        self.oDatabase.step01_save_parseiteration_fc(
            self.results_filename_str, self.current_timestamp_str, len(jobs), response_status_code, self.URL_START_STR
        )
